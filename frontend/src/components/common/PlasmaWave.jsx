...
export default function PlasmaWave(props) {
  ...
  useEffect(() => {
    ...
    // Visibility handling: pause rendering when offscreen or document hidden
    ...
    const visibilityHandler = () => {
      // no-op here; we read document.hidden directly in the loop
    };
    document.addEventListener("visibilitychange", visibilityHandler);

    // Cache color arrays to avoid re-parsing hex each frame
    const lastColors = [colors[0], colors[1]];
    const colorArr1 = new Float32Array(hexToRgb(lastColors[0]));
    const colorArr2 = new Float32Array(hexToRgb(lastColors[1]));
    program.uniforms.uColor1.value = colorArr1;
    program.uniforms.uColor2.value = colorArr2;
    ...
    const update = (now) => {
      ...
      // update only the values that change per-frame
      ...
      // update colors only when changed to avoid allocations
      if (cols[0] !== lastColors[0]) {
        const rgb = hexToRgb(cols[0]);
        colorArr1[0] = rgb[0];
        colorArr1[1] = rgb[1];
        colorArr1[2] = rgb[2];
        program.uniforms.uColor1.value = colorArr1;
      ...
      if (cols[1] !== lastColors[1]) {
        const rgb = hexToRgb(cols[1]);
        colorArr2[0] = rgb[0];
        colorArr2[1] = rgb[1];
        colorArr2[2] = rgb[2];
        program.uniforms.uColor2.value = colorArr2;
      ...
      // Only render when the element is visible and the document is not hidden
